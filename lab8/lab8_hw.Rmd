...
```{r}
sydneybeaches
#They appear to be long but tidy
```
...
```{r}
avg_by_year <- sydneybeaches_long2%>%
  group_by(year)%>%
  summarise_at(vars(enterococci_cfu_100ml),list(avg_pollution=mean),na.rm=T)
avg_by_year
```
...
```{r}
avg_by_year2 <-avg_by_year%>%
  pivot_wider(names_from = "year",values_from = "avg_pollution")
avg_by_year2
```
