...
```{r}
new_eagles <- mutate(conservation_status=ifelse(ecosphere$population_size<=250000,"Threatened","Not Threatened"))
```
...
Does habitat affect life expectancy? Which habitat has the greatest and lowest life expectancy?
```{r}
ecosphere%>%
  filter(habitat=="Grassland")%>%
  summary()
