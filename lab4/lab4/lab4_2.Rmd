...
#specifies specific values _ or __
```
...
#filters for observation with BOTH conditions
```

Notice that the `|` operator generates a different result.
```{r}
filter(fish, lakeid == "AL" | length > 350)
#filters for observations that meet one or both obervations
```
...
```{r}
mammals6 <-filter(mammals,genus=="Lophocebus"|genus=="Erythrocebus"|genus=="Macaca")
mammals6_updated <- select(mammals,"species","genus", "mass", "gestation","newborn")
```

```{r}
summary(mammals6_updated)
#min is -999.0, but this is impossible, must filter out negative numbers
```

```{r}
mammals6_final <- filter(mammals6_updated, newborn>0)
summary(mammals6_final)
#smallest newborn mass is the min. of the newborn, which is 0.2 but we need to know which species
```

```{r}
filter(mammals6_final, near(newborn, 0.2, tol=0.01))
#species with smallest newborn mass = Suncus etruscus
```
