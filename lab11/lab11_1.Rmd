...
```{r}
homerange%>%
  ggplot(aes(x=taxon,y=log10.mass,fill=taxon))+geom_boxplot()+theme(axis.text.x = element_text(angle = 60, hjust = 1))
```
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, #adds transparency
                 color = "deepskyblue4", #color of boundary around bins
                 fill = "deepskyblue4", #color of bins
                 bins=40)+
...
```{r}
homerange%>%
  ggplot(aes(x=log10.hra))+geom_histogram(aes(y=after_stat(density)),alpha=.4,color = "black",fill="orchid")+labs(title="Number of Observations based on log10.hra")+geom_density(color="orange")
```
...
homerange%>%
  ...
  ggplot(aes(x=taxon,y=log10.mass,fill=taxon))+geom_boxplot()+theme(axis.text.x = element_text(angle = 60, hjust = 1))+labs(title="Distribution of Mass of Small Ranged Taxa",x=NULL)
```
